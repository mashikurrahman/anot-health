$files = @('about.html','specialties.html','contact.html')
...
foreach ($f in $files) {
    ...
    # Update Header
    $c = $c -replace '<header class="header">[\s\S]*?</header>', $headerReplace
    $c = $c -replace '<header class="header scrolled">[\s\S]*?</header>', $headerReplace
    
    # Update Favicon
    $c = $c -replace '<link rel="icon" type="image/svg\+xml" href="images/favicon.svg">', '<link rel="icon" type="image/png" href="images/favicon.png">'
